// Copyright 2017 The AplaProject Authors
// This file is part of the AplaProject library.

// The AplaProject library is free software: you can redistribute it and/or modify
// it under the terms of the GNU Lesser General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.

// The AplaProject library is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
// GNU Lesser General Public License for more details.

// You should have received a copy of the GNU Lesser General Public License
// along with the AplaProject library. If not, see <http://www.gnu.org/licenses/>.

window.apla = (function () {
    ...
    var signAlg = 'SHA256withECDSA';
    var curveName = 'secp256r1';

    var generateSeed = function () {
        var result = [];
        for (var i = 0; i < 15; i++) {
            var value = randomEngine.pick(WORD_LIST);
            result.push(value);
        }
        return result.join(' ');
    };
    ...
    var generateKeyPair = function (seed) {
        var seedLower = seed.toLowerCase();
        var seedHex = '';
        for (var i = 0; i < seedLower.length; i++) {
            var char = seedLower[i];
        ...
        if (64 > seedHex.length) {
            var hash = KJUR.crypto.Util.sha256(seed);
            seedHex = hash.toString();
        }

        var curveParams = KJUR.crypto.ECParameterDB.getByName(curveName);
        var curveG = curveParams.G;
        var privateBig = new BigInteger(seedHex, 16);
        var publicBig = curveG.multiply(privateBig);
        var valueX = publicBig.getX().toBigInteger();
        var valueY = publicBig.getY().toBigInteger();
        var charLen = curveParams.keylen / 4;

        var privateHex = ('0000000000' + privateBig.toString(16)).slice(-charLen);
        var xHex = ('0000000000' + valueX.toString(16)).slice(-charLen);
        var yHex = ('0000000000' + valueY.toString(16)).slice(-charLen);
        var publicHex = '04' + xHex + yHex;
    ...
    var generateAll = function () {
        ...
        document.getElementById('key').value = keyPair.public;
    };
    ...
    document.getElementById('download').onclick = function () {
        download('apla_key.txt', generatePayload());
    };
